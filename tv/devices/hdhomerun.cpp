...
HDHomeRun::HDHomeRun(std::string ip_address): Device(ip_address, 3) {
    hdhomerun = 0;
}
HDHomeRun::~HDHomeRun() {
    cleanUp();
...
bool HDHomeRun::loadDevice() {
    ...
    for (int i=0; i<3; i++)
        this->tuners[i] = 0;
    for (int i=0; i<3; i++) {
        this->tuners[i] = new HDHomeRunTuner(this, i, tuners.at(i));
        if (((HDHomeRunTuner*)this->tuners[i])->getUPnPDeviceType().compare("urn:schemas-upnp-org:device:MediaServer:1")!=0) {
